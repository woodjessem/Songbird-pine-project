...
fig2<-fig2 + labs(x= "Type", y="Species Richness  (visit 1)") + theme(panel.grid.minor.x = element_blank()) + xlim(c("AC","PC")) + coord_cartesian(ylim=c(12, 34))+ scale_y_continuous(breaks=seq(10, 35, 4))
fig2
```
...
#interlude to break apart method, but still keep both years in
```{r}
methodYSR2pc <-read.csv("17_18_2by2_ACPC_SR_pc.csv") #SR by count #2 each
...
fig3 <- fig3 + scale_color_sjplot(palette = "metro ui") + scale_fill_sjplot(palette = "metro ui", discrete = TRUE, reverse = FALSE)
fig3

#fig3 <- plot_model(evaluationpc3d, type="pred", terms=c("Sdate"))
#fig3 <- fig3 + set_theme(base=theme_classic()) #scale_color_sjplot(palette = "metro ui") + scale_fill_sjplot(palette = "metro ui", discrete = TRUE, reverse = FALSE)
#fig3<- fig3 + labs(x= "Survey date (in season)", y="PC Species Richness  (visit 3)") + set_theme(base = theme_classic(), title.color = "white", scale_color_sjplot(palette = "deep reefs"), scale_fill_sjplot(palette = "deep reefs", discrete=TRUE, reverse=FALSE))
#fig3
...
print(qplot(x = Sdate, y = SR, color = "cadetblue", data = evaluationpc3d, geom = "smooth"))

print(plot(evaluationpc3d))
